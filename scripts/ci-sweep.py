...
Why not cargo-sweep itself: it reads atime through Rust's DirEntry::metadata,
which on Windows returns the timestamps cached in the parent directory's
listing rather than the file's own, and it swept 2.5 GiB of in-use units on
the windows runner. os.stat opens the file and reads the real value.
"""
...
def newest_atime_dirent(unit_dir: pathlib.Path) -> float:
    """The directory-listing view (what Rust's DirEntry::metadata sees on Windows)."""
    newest = 0.0
    with os.scandir(unit_dir) as it:
        for e in it:
            if e.is_file(follow_symlinks=False):
                newest = max(newest, e.stat(follow_symlinks=False).st_atime)
    return newest
    ...
    for fp_dir in sorted(TARGET.rglob(".fingerprint")):
        ...
        for unit in fp_dir.iterdir():
            ...
            if newest_atime(unit) >= stamp:
                keep.add(h)
            if windows and newest_atime_dirent(unit) >= stamp:
                dirent_keep += 1
        ...
        line = f"{profile}: {units} units, kept {len(keep)}, removed {removed} entries ({removed_bytes / 2**20:.1f} MiB)"
        if windows:
            line += f"; directory-listing view would keep {dirent_keep}"
        print(line)
